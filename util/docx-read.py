...
matrix_state = False
row_state = False 
word_num = 0 # 공백이 없는 Text Element에서 공백문자가 삽입되는 것을 방지하기 위한 변수
file_name = str()
...
while True :
    ...
    for node in tree_list:
        if node.tag == ns + 'tr' : # 행 구분 태그
            matrix_state = True
            if temp != '' : # 행 마무리시 temp에 저장된 text 출력
                out_process(str_init(temp, node.text, False, word_num))
                temp = 'tr에 의해서 초기화 됨'
        elif node.tag == ns + 'tc' : # 열 구분 태그
            ...
            temp = temp + '\t'
        elif node.tag == ns + 'bookmarkStart' : # 행렬 시작 태그
            out_process('-'*10 + '행렬시작' + '-'*10)
        elif node.tag == ns + 'bookmarkEnd': # 행렬 종료 태그
            matrix_state = False
            row_state = False
            out_process(str_init(temp, node.text, False, word_num))
            temp = '북마크엔드로 인하여 초기화됨'
